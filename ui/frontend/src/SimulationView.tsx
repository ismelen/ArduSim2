...
const SplitButton: React.FC<SplitButtonProps> = ({
  ...
  return (
    <div
      className={`split-btn-group ${disabled ? "disabled" : ""} ${activeClass}`}
      ref={dropdownRef}
    >
      <button
...
const SimulationView: React.FC = () => {
  ...
  const [pausedAlgorithms, setPausedAlgorithms] = useState<
    Record<string, boolean>
  >({});
  const [startedAlgorithms, setStartedAlgorithms] = useState<
    Record<string, boolean>
  >({});
  ...
  fleetUavs.forEach((u) =>
    u.services?.forEach((s) => runningAlgorithms.add(s.serviceId)),
  );
  ...
  const allUavsReady =
    fleetUavs.length > 0 &&
    fleetUavs.every((fu) => uavs[fu.id] && uavs[fu.id].nrGpsOnline > 0);
  ...
  useEffect(() => {
    ...
    uavList.forEach((uav) => {
      ...
      if (markersRef.current[uav.id]) {
        ...
        // Create new marker
        const el = createUAVMarkerElement(
          uav.heading,
          uav.alt,
          getColorForUav(uav.id),
        );
  ...
  useEffect(() => {
    ...
    EventsOn("simulation:log", (message: string) => {
      ...
      setLogs((prev) => [
        ...prev.slice(-49),
        { time: timeStr, level: "[INF]", msg: message },
      ]);
    });
    ...
    EventsOn("netsim:message", (data: NetSimMessageEvent) => {
      ...
      setLogs((prev) => [
        ...prev.slice(-49),
        { time: timeStr, level: "[MSG]", msg: data.label },
      ]);
    });
    ...
    <div className="sim-container">
        ...
        <div className="sim-exiting-overlay">
          <div className="exiting-content">
            ...
            <div className="exiting-text-group">
              ...
              <div className="exiting-subtitle">
                Removing containers and cleaning environment...
              </div>
            </div>
          </div>
        </div>
          ...
          {algorithmsList.length > 0 ? (
            <>
              <SplitButton
                ...
                activeClass={
                  allUavsReady && !allStarted ? "btn-solid-ready" : ""
                }
      ...
      <div className="sim-log-panel">
        <div className="panel-header">
          <span className="material-symbols-outlined log-header-icon">
            terminal
          </span>
        ...
        <div className="log-content">
          {logs.length === 0 ? (
            <div className="log-entry">
              ...
              <span className="log-level info">[WAIT]</span>{" "}
              AWAITING_SIMULATION_UPLINK...
            </div>
          ) : (
            logs.map((log, i) => {
            ...
            return (
              ...
              >
                <div className="card-label">
                  UAV {uav.id}{" "}
                  <span
                    className={`status-dot ${uav.nrGpsOnline > 0 ? "ready" : "not-ready"}`}
                  ></span>
                </div>
                ...
                <div className="compact-stats">
                  <div className="stat">
                    ...
                    <span>
                      {uav.alt.toFixed(1)} <small>m</small>
                    </span>
                  </div>
                  <div className="stat">
                    <span className="material-symbols-outlined">speed</span>
                    <span>
                      {Math.sqrt(uav.vx ** 2 + uav.vy ** 2).toFixed(1)}{" "}
                      <small>m/s</small>
                    </span>
                  </div>
                  <div className="stat">
                ...
                >
                  <span style={{ color: "var(--on-surface)" }}>LAT</span>{" "}
                  {uav.lat.toFixed(5)}° &bull;{" "}
                  <span style={{ color: "var(--on-surface)" }}>LNG</span>{" "}
                  {uav.lon.toFixed(5)}° &bull;{" "}
                  <span style={{ color: "var(--uav-color)" }}>HDG</span>{" "}
                  {uav.heading.toFixed(0)}°
                </div>
